...
class FileUtils extends AbstractSingleton
{
    ...
    public function recurseCopy( $sourceDirectory, $destinationDirectory, $childFolder = '') {
        ...
        if ($childFolder !== '') {
            if (is_dir("$destinationDirectory/$childFolder") === false) {
                $success=mkdir("$destinationDirectory/$childFolder");
                if(!$success){
                    return false;
                }
            }
            ...
            while (($file = readdir($directory)) !== false) {
                ...
                if (is_dir("$sourceDirectory/$file") === true) {
                    $success=$this->recurseCopy("$sourceDirectory/$file", "$destinationDirectory/$childFolder/$file");
                    if(!$success){
                        return false;
                    }
                } else {
                    $success=copy("$sourceDirectory/$file", "$destinationDirectory/$childFolder/$file");
                    if(!$success){
                        return false;
                    }
                }
        ...
        while (($file = readdir($directory)) !== false) {
            ...
            if (is_dir("$sourceDirectory/$file") === true) {
                $success=$this->recurseCopy("$sourceDirectory/$file", "$destinationDirectory/$file");
                if(!$success){
                    return false;
                }
            }
            else {
                $success=copy("$sourceDirectory/$file", "$destinationDirectory/$file");
                if(!$success){
                    return false;
                }
            }
     ...
     * @param $dirPath
     * @return boolean|null renvoie null si le répertoire n'existe pas, true si il a été supprimé, false si une erreur s'est produite
     */
            ...
            foreach ($objects as $object) {
                if ($object != "." && $object !="..") {
                    if (filetype($dirPath . DIRECTORY_SEPARATOR . $object) == "dir") {
                        $success=$this->removeDir($dirPath . DIRECTORY_SEPARATOR . $object);
                        if($success === false){
                            return false;
                        }
                    } else {
